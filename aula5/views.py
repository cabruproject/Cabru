...
def criar():
    ...
    timestamp = time.time()
    arquivo.save(f'{upload_path}/capa{aluno.id_aluno}-{timestamp}.jpg')
...
def atualizar():
    ...
    turma_fk = request.form['turma_fk']
    desconto = request.form['desconto']
    aluno = Aluno(nome, sexo, telefone, email, data_nascimento, data_matricula, turma_fk, desconto, id_aluno=request.form['id_aluno'])

    arquivo = request.files['arquivo']
    upload_path = app.config['UPLOAD_PATH']
    timestamp = time.time()
    deleta_arquivo(aluno.id_aluno)
    arquivo.save(f'{upload_path}/capa{aluno.id_aluno}-{timestamp}.jpg')
